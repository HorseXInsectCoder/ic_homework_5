import { ic_homework_5_backend } from "../../declarations/ic_homework_5_backend";
...
async function post() { 
  ...
  try{
    await ic_homework_5_backend.post(opt, textarea.value);
    textarea.value = "";
  }catch(err){
    console.log(err);
...
async function set_name() { 
  ...
  let opt = document.getElementById("opt").value;
  try{
    await ic_homework_5_backend.set_name(opt, textarea.value);
  }catch(err){
    console.log(err);
...
async function follow() { 
  ...
  try{
    await ic_homework_5_backend.follow(Principal.fromText(textarea.value));
  }catch(err){
    console.log(err);
    error.innerText = "Set Failed!";
  }
...
async function reflesh_author_list(){
  let followed = await ic_homework_5_backend.follows();
...
async function load_posts(author) {
  ...
  switch(author) {
    case "0":
      posts = await ic_homework_5_backend.posts(1000);
      break;
    case "1":
      posts = await ic_homework_5_backend.timeline([], 1000);
      break;
    default:
      posts = await ic_homework_5_backend.timeline([Principal.fromText(author)], 1000);
  }
  console.log(posts);
